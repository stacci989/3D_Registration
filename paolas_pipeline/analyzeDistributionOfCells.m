...
folder2save = 'W:\scratch\garber\thorstac\Thesis_examples\Drafts\Corel\Figure2_Antero';
mkdir(folder2save)
cd(folder2save)
...
%                   & S(i).T_roi.ML_location <= 0;       %  Note the ()s.
    ...
    p(i) = plot3(ap_pixel, ml_pixel, dv_pixel, '.','linewidth',10, 'color', [.5 0 .5], 'markers',4) 
end

%fig2svg('myfile.svg', 'plot3', 'svg')
...
if makeVideo
    view([-90, 8]);
    WriterObj = VideoWriter('ntsr1.mp4', 'MPEG-4');
    WriterObj.FrameRate=30;
    open(WriterObj);
    for f = 8:20
        view([-90 f])
